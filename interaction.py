...
def library_from_path(path, word_length):
    word_re = r"^[a-zA-Z]+$"
    ...
    return WordSuggester(valid_words)


def get_suggestions(library, suggestion_count):
    print("Getting suggestions...")

    start = time.time()
    suggestions = library.suggest_words(suggestion_count)
    end = time.time()
    duration = end - start
...
def get_user_feedback(word_length):
    while True:
